...
let check_direction_of_char row pos = 
  ...
  (List.nth row (pos-1) = '0' && List.nth row (pos+1) = '0')
...
let rec transpose_matrix = function
  | [] -> []
  | []::x1  -> transpose_matrix x1 
  | (x::x1)::x2 -> (x :: List.map List.hd x2) :: transpose_matrix (x1 :: List.map List.tl x2)
...
let get_spaces (num: int) (row: char list) (coord: int*int) (dir: bool): letter = 
  let rec get_spaces_helper num curr row space1 space2 temp = 
    ...
    |h::t when curr < num -> 
      if (h = '0') then get_spaces_helper num (curr + 1) t (space1 + 1) space2 temp
      else get_spaces_helper num (curr + 1) t 0 space2 temp
    |h::t when (curr = num) ->  get_spaces_helper num (curr + 1) t space1 space2 temp
    |h::t when (h = '0' && temp) -> get_spaces_helper num (curr + 1) t space1 (space2+1) temp
    |_ -> get_spaces_helper num (curr + 1) [] space1 space2 false 
    ...
    |h1::t1 -> 
      get_list_1 (idx + 1)
        ((let rec get_list_2 num acc (row: char list) = 
            match row with 
            |[] -> acc
            |h::t when h <> '0' -> get_list_2 
                                     (num+1) ((if (check_direction_of_char h1 num) 
                                               then (get_spaces num h1 (idx, num) (check_direction_of_char h1 num)) 
                                               else (get_spaces idx (List.nth transposed num) (idx, num))(check_direction_of_char h1 num) )::acc) t 
            |h::t -> get_list_2 (num+1) acc t in 
...
let contains_minus_1 word inventory = 
  let rec contains_minus_1_helper word1 inventory1 pepito pos =
    match word1 with 
    |[] -> (true, (pepito, pos))
    |h::t when List.mem (Char.uppercase_ascii h) inventory1 -> 
      let x = (remove_character (Char.uppercase_ascii h) inventory1 []) in contains_minus_1_helper t x
        pepito ((if (pepito = '0' && h <> '0') then (pos+1) else pos))
    |h::t when (pepito = '0' && h <> '0') ->  contains_minus_1_helper t inventory1 h pos 
    |_ -> (false, (pepito, pos)) in 
...
let rec pepitototo ter dict acc = 
  match dict with 
  |[] -> acc
  |h::t when (String.length h > 0) && (Char.uppercase_ascii h.[0]) = ter -> pepitototo ter t (h::acc) 
  |h::t -> pepitototo ter t acc
...
let rec find_board_letters c pos len board = function 
  |[] -> (false, (true,(0,0)))
  |h::t1 when (Char.uppercase_ascii c) = (get_char_2d h.coord board) && (h.space1 > pos) && (h.space2 > (len - 1 - pos)) -> 
    let t = get_coord h.dir h.coord pos in if check_surroundings h.dir t len board true then (true, (h.dir, t)) 
    else find_board_letters c pos len board t1
  |h::t1 -> find_board_letters c pos len board t1
      ...
      in if fst x && (String.length h <> 1) then 
        ...
        in if fst y then 
          (match curr with
           ...
           |Some z when (String.length h > String.length (fst z)) -> find_word_helper board t board_ters inventory
                                                                       (Some (String.uppercase_ascii h, snd y))
           |Some z -> find_word_helper board t board_ters inventory (Some z))
  ...
  let rec get_ai_word_helper dict inventory curr = 
    ...
    |h::t -> 
      ...
      |Some x -> match curr with 
        ...
        |Some y when (String.length (fst x) > String.length (fst y)) -> get_ai_word_helper dict t (Some x) 
        |_ -> get_ai_word_helper dict t curr
...
let get_missing_letter_pos (word:char list) (dir: bool) (curr: int*int) (board: (char list) list)= 
  let rec helper word num dir board = 
    match word with 
    |[] -> (-1)
    |h::t when dir -> if (get_char_2d ((fst curr), ((snd curr) + num)) board) <> '0' then num
...
let is_valid (word:char list) st pos = 
  let rec is_valid_helper word inventory num pos temp = match word with 
    ...
    |h::t when List.mem h inventory -> is_valid_helper t 
                                         (if (h <> pos || temp <> 0) then (remove_character h inventory []) 
                                          else inventory) num pos (if h = pos then 1 else temp)   
    |h::t when num = 0 -> is_valid_helper t inventory 1 pos temp 
    |_ -> (false, (inventory, num)) in 
  ...
  let rec find_word_helper_first board words inventory 
    ...
    |h::t -> 
      ...
      if x && (String.length h <> 1) then match curr with
        ...
        |Some y when (String.length h > String.length (fst y)) -> find_word_helper_first board t inventory
                                                                    (Some (String.uppercase_ascii h, (true, (8,8))))
        |Some y -> find_word_helper_first board t inventory (Some y)
  ...
  let rec get_ai_word_helper_first dict inventory curr = 
    ...
    |h::t -> match find_word_first board (pepitototo h dict []) (snd(ai)) with 
      ...
      |Some x -> match curr with 
        ...
        |Some y when (String.length (fst x) > String.length (fst y)) -> get_ai_word_helper_first dict t (Some x) 
        |_ -> get_ai_word_helper_first dict t curr
